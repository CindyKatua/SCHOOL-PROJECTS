...
class CafeTruckApp(tk.Tk):
    def __init__(self):
        ...
        self.configure(bg='#F7E0DE')

        self.welcome_page()

    def welcome_page(self):
        self.label = tk.Label(self, text='Hello!\n WELCOME TO OUR COFFEE SHOP ', bg='#F7E0DE', fg='#525F4F', font=('Times New Roman', 30))
        self.label.pack(pady=10)

        self.entry_name = tk.Entry(self)
        self.entry_name.pack(pady=10)  

        self.start_button = tk.Button(self, text='START ORDERING', bg='#F2EBD9', fg='#692E1D', font=('Century Regular', 16), command=self.start_ordering)
    ...
    def start_ordering(self):
        ...
        self.label.config(text='OPTIONS', bg='#F7E0DE', fg='#525F4F', font=('Century Regular', 30))
        ...
        self.button1 = tk.Button(self, text='Read the Menu', bg='#F2EBD9', fg='#692E1D', font=('Century Regular', 16), command= self.open_menu)
        self.button1.pack(pady=10)

        self.button2 = tk.Button(self, text='To order', bg='#F2EBD9', fg='#692E1D', font=('Century Regular', 16),command=self.order_page)
        self.button2.pack(pady=10)

        self.button3 = tk.Button(self, text='To see your total', bg='#F2EBD9', fg='#692E1D', font=('Century Regular', 16), command=self.show_total)
        self.button3.pack(pady=10)

        self.button4 = tk.Button(self, text='To confirm order and exit', bg='#F2EBD9', fg='#692E1D', font=('Century Regular', 16),command=self.confirm_exit)
    ...
    def open_menu(self):
        ...
        menu_window.configure(bg='#F2EBD9')
        ...
        for option in menu_option:
            option = option.strip()
            menu_label = tk.Label(menu_window, text=option, bg='#F2EBD9',fg='#692E1D', font=('Times New Roman',10))
            menu_label.pack() 

    def order_page(self):
        order_window = tk.Tk()
        order_window.title("ORDER PAGE.")
        order_window.geometry('440x640')
        order_window.configure(bg='#F7E0DE')
        label_text = '''
...
'''
        label = tk.Label(order_window, text = label_text, bg='#F7E0DE', fg='#525F4F', font=('Times New Roman', 10))
        label.grid(row=0, column=0, columnspan=2, sticky="w")

        self.order_label = tk.Label(order_window, text = ("Enter order no:") , bg='#F7E0DE',fg='#692E1D', font=('Century Regular', 10))
        ...
        self.quantity_label = tk.Label(order_window, text = "How many items of this order would you like?:",bg='#F7E0DE',fg='#692E1D', font=('Century Regular', 10))
        ...
        orderingbutton = tk.Button(order_window, text = "Add order", command = self.add_order,bg='#F2EBD9', fg='#692E1D', font=('Century Regular', 10))
        orderingbutton.grid(row=3, column=0, columnspan=2, pady=10)

        exit_button = tk.Button(order_window, text="Done", command=order_window.destroy,bg='#F2EBD9', fg='#692E1D', font=('Century Regular', 10))
    ...
    def show_total(self):
        ...
        total_window.config(bg='#F7E0DE')
        tk.Label(total_window, text=f"Total: {sum(total)} TL",bg='#F7E0DE', fg='#692E1D', font=('Times New Roman', 16)).place(x=50,y=70)     
